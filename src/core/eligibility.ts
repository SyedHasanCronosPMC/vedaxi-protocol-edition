import { LEVELS, type ElasticState, type ElementId, type ToolName } from './types'
...
const atLeast = (state: ElasticState, id: ElementId, level: keyof typeof LEVELS) => LEVELS[state.attention[id]] >= LEVELS[level]
const focusedAssets = (state: ElasticState) => (Object.keys(state.attention) as ElementId[]).filter((id) => LEVELS[state.attention[id]] === LEVELS.FOCUSED)
...
export const CAPABILITY_RULES: Record<Exclude<ToolName, typeof SPINE_TOOLS[number]>, (state: ElasticState) => boolean> = {
  ...
  create_citation: (s: ElasticState) => s.attention.paper === 'FOCUSED',
}
...
export function focusedIds(state: ElasticState): ElementId[] { return focusedAssets(state) }
